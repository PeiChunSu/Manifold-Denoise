function [Y_os,eta,r_p,k,fZ] = optimal_shrinkage_color5(Y,loss,method,C,c)
...
lambda_p = [];
for i = 0:19
    lambda_p = [lambda_p,lab(k+i+1)+ 1.1/(2^(2/3)-1) * (lab(k+i+1)-lab(2*(k+i)+1))];
end
lambda_p = mean(lambda_p);
%}
lambda_p = lab(k+1)+ 1.1/(2^(2/3)-1) * (lab(k+1)-lab(2*(k)+1));
xx = abs(lab(1:k)-lambda_p);
%r_p = max(find(xx(1:(end-1))./xx(2:end)>p^(1/3)))
%r_p = min(find(log(xx)/log(p)<0))-1;
r_p = sum(lab-lambda_p>C*p^(-1/3));
%r_p = sum(lab>lambda_p);
